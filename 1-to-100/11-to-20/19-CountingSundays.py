...
""" # 1 # Solution, this solution causes "Runtime Error" error in some tests:
import datetime as dt
st, en = [list(map(int, input().strip().split())), list(map(int, input().strip().split()))]
start, end = dt.datetime(st[0], st[1], st[2]), dt.datetime(en[0], en[1], en[2])
while start.isoweekday() != 7:
    start = start + dt.timedelta(1)

k = 0
while not (start.year == end.year and start.month == end.month):
    if start.day == 1: k += 1
